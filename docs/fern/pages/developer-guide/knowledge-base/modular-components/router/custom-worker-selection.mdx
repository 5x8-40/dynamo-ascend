...
Implement `RequestClassifier` in your policy crate. Its provider validates `RequestClassifierParameters` at startup and returns a `RequestClassifierFactory`: a closure that creates a fresh `Box<dyn RequestClassifier>` for each routed model. Register that provider alongside your worker-selection providers:

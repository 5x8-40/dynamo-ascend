...
@pytest.mark.parametrize(
    ...
    [
        ({}, None),
        ({"agent_context": {}}, None),
        ({"agent_context": {"session_id": "session-1"}}, "session-1"),
    ],
)
def test_session_id_from_request(raw_request, expected):
    from dynamo.vllm.handlers import _session_id_from_request

    assert _session_id_from_request(raw_request) == expected
